<?php
	...
	<body style=\"font-family:calibri; font-size: 14px; color: #333;\">
		...
		<p style=\"font-weight:700\">Your Promo Code is ".$promoCode."</p>
	...
	$headers .= 'Cc: biswajit.swain@haastika.com' . "\r\n";

	// mail($emailAddr,$subject,$message,$headers);

	if(@mail($emailAddr,$subject,$message,$headers)) {
	  	$mailSentStatus ['status'] = true;
		$mailSentStatus ['message'] = "Mail sent successfully !!";
	} else {
	  	$mailSentStatus ['status'] = false;
		$mailSentStatus ['message'] = "Mail sending failed ! Please try once again.";
	}
